...
from src.utils.mesh_util import save_obj, save_obj_with_mtl, save_glb, save_glb_with_mtl
...
def make_mesh(mesh_fpath, planes, model, infer_config, export_texmap, export_type):
    ...
    with torch.no_grad():
        ...
        if export_type == 'obj':
            if export_texmap:
                ...
                print(f"OBJ Mesh with texmap saved to {mesh_fpath}")
            else:
                ...
                print(f"OBJ Mesh saved to {mesh_fpath}")
        else:
            if export_texmap:
                vertices, faces, uvs, mesh_tex_idx, tex_map = mesh_out
                save_obj_with_mtl(
                ...
                print(f"GLB Mesh with texmap saved to {mesh_vis_fpath}")
            else:
                vertices, faces, vertex_colors = mesh_out
                vertices = vertices[:, [1, 2, 0]]
                vertices[:, -1] *= -1
                faces = faces[:, [2, 1, 0]]
                save_glb(vertices, faces, vertex_colors, mesh_vis_fpath)
                print(f"GLB Mesh saved to {mesh_vis_fpath}")

    if export_type == 'obj':
        return mesh_fpath
    else:
        return mesh_vis_fpath

def make3d(images, model, device, IS_FLEXICUBES, infer_config, export_video, export_texmap, export_type):
    ...
    mesh_fpath = make_mesh(mesh_fpath, planes, model, infer_config, export_texmap, export_type)
    if export_video:
...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        export_type: str = Input(default="obj"),
    ) -> List[Path]:
        ...
        if export_video:
            if export_type == 'obj':
                output_video, output_model_obj = make3d(mv_images, self.model, self.device, self.IS_FLEXICUBES, self.infer_config, export_video, export_texmap, export_type)
                if export_texmap:
                ...
                output_video, output_model_glb = make3d(mv_images, self.model, self.device, self.IS_FLEXICUBES, self.infer_config, export_video, export_texmap, export_type)
                return [Path('/content/InstantMesh/mv_show_images.png'), Path(output_video), Path(output_model_glb)]

        else:
            if export_type == 'obj':
                output_model_obj = make3d(mv_images, self.model, self.device, self.IS_FLEXICUBES, self.infer_config, export_video, export_texmap, export_type)
                if export_texmap:
                    mesh_basename = os.path.splitext(output_model_obj)[0]
                ...
                output_model_glb = make3d(mv_images, self.model, self.device, self.IS_FLEXICUBES, self.infer_config, export_video, export_texmap, export_type)
                return [Path('/content/InstantMesh/mv_show_images.png'), Path(output_model_glb)]